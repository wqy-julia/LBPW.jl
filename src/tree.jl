struct LBPWTree{B,A,O,RB}
    ...
    function LBPWTree{B,A,O,RB}(root_belief, sz::Int=1000) where{B,A,O,RB}
        sz = min(sz, 100_000)
        return new(
            sizehint!(Int[], sz),
            sizehint!(Int[], sz),
...
@inline function push_anode!(tree::LBPWTree{B,A,O}, h::Int, a::A, n::Int=0, v::Float64=0.0, update_lookup=true) where {B,A,O}
...
struct LBPWTreeObsNode{B,A,O,RB} <: BeliefNode
    tree::LBPWTree{B,A,O,RB}
    node::Int
end

isroot(h::LBPWTreeObsNode) = h.node==1
@inline function belief(h::LBPWTreeObsNode)
    if isroot(h)
...
function sr_belief(h::LBPWTreeObsNode)
    if isroot(h)
        error("Tried to access the sr_belief for the root node in a LBPW tree")
    else
        return h.tree.sr_beliefs[h.node]
    end
end
n_children(h::LBPWTreeObsNode) = length(h.tree.tried[h.node])
