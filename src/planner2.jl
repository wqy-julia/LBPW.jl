mutable struct LBPWPlanner{P,NBU,C,NA,SE,IN,IV,SolverType} <: Policy
    solver::SolverType
    problem::P
    ...
    init_N::IN
    init_V::IV
    tree::Union{Nothing, LBPWTree} # this is just so you can look at the tree later
end

function LBPWPlanner(solver, problem::POMDP)
    LBPWPlanner(solver,
                  problem,
...
Random.seed!(p::LBPWPlanner, seed) = Random.seed!(p.solver.rng, seed)

function action_info(pomcp::LBPWPlanner{P,NBU}, b; tree_in_info=false) where {P,NBU}
    A = actiontype(P)
...
action(pomcp::LBPWPlanner, b) = first(action_info(pomcp, b))

function POMDPPolicies.actionvalues(p::LBPWPlanner, b)
    tree = make_tree(p, b)
    search(p, tree)
...
function make_tree(p::LBPWPlanner{P, NBU}, b) where {P, NBU}
    S = statetype(P)
    A = actiontype(P)
    O = obstype(P)
    B = belief_type(NBU,P)
    return LBPWTree{B, A, O, typeof(b)}(b, 2*min(100_000, p.solver.tree_queries))
    # return LBPWTree{B, A, O, typeof(b)}(b, 2*p.solver.tree_queries)
end


function search(pomcp::LBPWPlanner, tree::LBPWTree, info::Dict{Symbol,Any}=Dict{Symbol,Any}())
    all_terminal = true
    # gc_enable(false)
    i = 0
    start_us = CPUtime_us()
    ...
    while i < pomcp.solver.tree_queries
        ...
        if !POMDPs.isterminal(pomcp.problem, s)
            ...
            reward, depth = simulate(pomcp, LBPWTreeObsNode(tree, 1), s, max_depth)
    ...
    best_node = select_best(pomcp.solver.final_criterion, LBPWTreeObsNode(tree,1), pomcp.solver.rng)
